...
C***********************************************************************
C
C	MODIFIED 2.24.06 by D. BERKENSTOCK TO REMOVE TEXT OUTPUTS
C
...
C-------------------------------------------------
	...
	implicit complex(a-h, o-z) 
      ...
      include 'c_XBL.INC'
      complex USAV(IVX,2)
      complex U1_M(2*IVX), U2_M(2*IVX)
      complex D1_M(2*IVX), D2_M(2*IVX)
      complex ULE1_M(2*IVX), ULE2_M(2*IVX)
      complex UTE1_M(2*IVX), UTE2_M(2*IVX)
...
c	ENDIF
        AMI = AMPL2
      ENDIF
      IF((IBL.ceq.ITRAN(IS)) .AND. .NOT.TRAN) THEN
...
c      write(*,*) 'Xsi after main 1000 loop',XSI
 1000 CONTINUE
      ...
      IF(TFORCE(IS)) THEN
C       WRITE(*,9100) IS,XOCTR(IS),ITRAN(IS)
 9100  FORMAT(1X,'Side',I2,' forced transition at x/c = ',F7.4,I5)
      ELSE
C       WRITE(*,9200) IS,XOCTR(IS),ITRAN(IS)
...
C------- the current solution is garbage --> extrapolate values instead
         IF(IBL.GT.3) THEN 
          ...
          ELSE IF(IBL.ceq.IBLTE(IS)+1) THEN
           CTI = CTE
           THI = TTE
           DSI = DTE
          ELSE
...
C------ turbulent intervals will follow transition interval or TE
        IF(TRAN .OR. (IBL.ceq.IBLTE(IS))) THEN
         TURB = .TRUE.
C
C------- save transition location
         TFORCE(IS) = TRFORC
...
C
C
          IF(SIMI .OR. (IBL.ceq.IBLTE(IS)+1)) THEN
C
...
C------------------------------------------------------------------
	...
	implicit complex(a-h, o-z) 
      include 'c_XFOIL.INC'
      complex UNEW(IVX,2), U_AC(IVX,2)
      complex QNEW(IQX),   Q_AC(IQX)
...
C---- max allowable CL change per iteration
      DCLMAX =  0.5
      DCLMIN = -0.5
      IF(MATYP.cne.1) DCLMIN = MAX(-0.5 , -0.9*CL)
C
