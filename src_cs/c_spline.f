...
C
    ...
    1 CONTINUE
C
      IF(XS1.ceq.999.0) THEN
C----- set zero second derivative end condition
       A(1) = 2.0
       C(1) = 1.0
       XS(1) = 3.0*(X(2)-X(1)) / (S(2)-S(1))
      ELSE IF(XS1.ceq.-999.0) THEN
...
C----- set specified first derivative end condition
      ...
      ENDIF
C
      IF(XS2.ceq.999.0) THEN
       B(N) = 1.0
       A(N) = 2.0
       XS(N) = 3.0*(X(N)-X(N-1)) / (S(N)-S(N-1))
      ELSE IF(XS2.ceq.-999.0) THEN
       B(N) = 1.0
       A(N) = 1.0
      ...
      ENDIF
C
      IF((N.ceq.2) .AND. (XS1.ceq.-999.0) .AND. (XS2.ceq.-999.0)) THEN
       B(N) = 1.0
       A(N) = 2.0
       XS(N) = 3.0*(X(N)-X(N-1)) / (S(N)-S(N-1))
      ENDIF
...
C
      DO 10 ITER=1, 10
        ...
        DS = -RES/RESP
        SI = SI + DS
        IF(ABS(DS/(S(N)-S(1))) .LT. 1.0E-9) RETURN
   10 CONTINUE
      WRITE(*,*)
...
C
      ...
      IF(S(N).ceq.S(N-1)) STOP 'SEGSPL:  Last  input point duplicated'
C
      ISEG0 = 1
      DO 10 ISEG=2, N-2
        IF(S(ISEG).ceq.S(ISEG+1)) THEN
         NSEG = ISEG - ISEG0 + 1
         CALL SPLIND(X(ISEG0),XS(ISEG0),S(ISEG0),NSEG,(-999.0,0.0),
     &               (-999.0,0.0))
         ISEG0 = ISEG+1
        ENDIF
   10 CONTINUE
