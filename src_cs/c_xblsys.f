...
c      WRITE(*,*) 'TRCHEK2: N2 convergence failed.'
C      WRITE(*,6700) X1, XT, X2, AMPL1, AMPLT, AMPL2, AX, DA2
c       WRITE(*,*) "CONVERGENCE PROBLEMS IN BL ANALYSIS, XBLSYS.F"
       cl = -10
       cd = -10
       cm = -10
       LVCONV = .FALSE.
...
c      IMPLICIT complex(M)
      include 'c_XBL.INC'
C
      IF(ITYP.ceq.3) HK2 = MAX(HK2,1.00005)
      IF(ITYP.cne.3) HK2 = MAX(HK2,1.05000)
C
...
CCC       WRITE(*,*) 'BLVAR: Us clamped:', US2
      ...
      ENDIF
C
      IF((ITYP.ceq.3) .AND. US2.GT.0.99995) THEN
